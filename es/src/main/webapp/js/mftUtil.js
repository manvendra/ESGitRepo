	...
	return {
		...
		getExpenseItemJsonFromForm : function(formName) {
			...
			for (var i = 0; i < expenseItemObject.paidBy.length; i++) {
				...
				var amount = $('#expenseForm input[_pByid^=' + paidBy.participantId + ']').val();
				paidBy.expenseAmont = (amount == "")?0:amount;

			}
			for (var i = 0; i < expenseItemObject.paidFor.length; i++) {
				var paidFor = expenseItemObject.paidFor[i];
				var amount = $('#expenseForm input[_pForid^=' + paidFor.participantId + ']').val();
				paidFor.expenseAmont = (amount == "")?0:amount;
